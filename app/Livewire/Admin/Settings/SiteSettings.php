...
class SiteSettings extends Component
{
    ...
    {
        $this->fillGroup($settingService->getGroupValues('general'));
        $this->fillGroup($settingService->getGroupValues('contact'));
        $this->fillGroup($settingService->getGroupValues('social'));
        $this->fillGroup($settingService->getGroupValues('seo'));
    }

    /**
     * Assign a flat ['key' => 'value'] array to matching component properties.
     * Image keys are stored in *_current properties; upload properties stay null.
     */
    private function fillGroup(array $values): void
    {
        $imageKeys = ['general_logo', 'general_favicon', 'seo_og_image'];

        foreach ($values as $key => $value) {
            // Ensure key is a string (never numeric)
            if (! is_string($key) || $key === '') {
                continue;
            }

            if (in_array($key, $imageKeys)) {
                $currentProp = $key.'_current';
                if (property_exists($this, $currentProp)) {
                    $this->{$currentProp} = ($value !== '' && $value !== null) ? $value : null;
                }
            ...
            if (property_exists($this, $key)) {
                $this->{$key} = $value; // already cast to string in service
            }
        }
    }
